...
(function(){
  ...
  function getGioHoangDao(jd){
    ...
    for (let i=0;i<12;i++){
      if (gioHD.charAt(i) === '1'){
        ret += CHI[i] + ' <b style="color:orange;">(' + ((i*2+23)%24) + '-' + ((i*2+1)%24) + ')</b>';
        if (count++ < 5) ret += ", ";
      }
    }
    return ret;
  ...
  function printTable(mm, yy, today){
	...
	const bgUrl = new URL(`./${bg}`, import.meta.url).href;
    ...
    res += `<div class="thangnam_amlich" style="text-align:center;">${thangAm}</div>`;
    res += `<div class="ngayamlich">${currentLunarDate.day}</div>`;
    res += `<div class="thangnam_amlich" style="text-align:center; line-height:160%;">${getYearCanChi(currentLunarDate.year)} (${currentLunarDate.year})</div>`;
    res += '</td>';
