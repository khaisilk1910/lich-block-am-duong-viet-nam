...
// ===== Ngũ hành nạp âm (60 hoa giáp) =====
  ...
  function getGioHacDao(jd){
    ...
    if (!this.config.show_svg) return '';
...
// ===== Hướng Xuất Hành (theo Can ngày) =====
  ...
  function printTable(mm, yy, today, bgrOpacity){
    ...
    const svgNgay  = this.config.show_svg ? this.getSvgConGiap(lunarDayIndex)  : '';
    const svgThang = this.config.show_svg ? this.getSvgConGiap(lunarMonthIndex) : '';
    const svgNam   = this.config.show_svg ? this.getSvgConGiap(lunarYearIndex) : '';


    res += `<tr>`;
    if (this.config.show_svg) {
      res += `<td class="svg-cell" style="transform: scaleX(-1);">${svgNgay}</td>`;
    } else {
      res += `<td class="svg-cell svg-hidden"></td>`;
    }
